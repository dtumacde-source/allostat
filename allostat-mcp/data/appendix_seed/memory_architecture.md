...
- **Injection (core view):** the behavioral layer injects in full, with each `feedback_*` entry individually addressable but its hook tail tightened to ≤8 words. The `## Project` and `## Reference` bodies collapse to a single `## <Name> (N) — full list in MEMORY.md (read on demand)` header plus an explicit on-disk pointer to the full index. There is a hard char-cap backstop (truncate-with-pointer), so the injected core never grows unbounded.
